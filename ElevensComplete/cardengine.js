...
function shuffleDeck(deck)
{
	...
	*/
	while (deck.length > 0)
	{
		...
		var removedCardArray = deck.splice(idx, 1);
		var card = removedCardArray[0];
...
function forwardSwapShuffle(deck)
{
	for (var i = 0; i < deck.length - 1; i++)
	{
		var idx = getRandomInteger(i, deck.length - 1);
		var temp = deck[i];
		deck[i] = deck[idx];
		deck[idx] = temp;
	}
